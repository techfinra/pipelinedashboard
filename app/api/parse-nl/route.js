...
오늘 날짜는 ${today}입니다. "다음주 월요일", "추석 이후", "이번주 금요일"처럼 상대적인 날짜 표현이 나오면 반드시 이 오늘 날짜를 기준으로 정확히 계산하세요. 연도를 임의로 추측하지 말고, 오늘 날짜의 연도를 기준으로 계산하세요.
...
13. nextActionDate (다음 액션 실행 예정일): 다음 액션에 날짜/기한이 명확히 언급되면 위 12번과 별도 항목으로 제안 (다음 액션 텍스트 자체는 안 바뀌어도 날짜만 새로 언급되면 이것만 제안 가능). **suggestedValue는 반드시 YYYY-MM-DD 형식이어야 합니다 (예: "2026-09-22"). "다음주 월요일" 같은 표현을 그대로 쓰지 말고 위의 오늘 날짜를 기준으로 정확히 계산한 날짜로 변환하세요.**
